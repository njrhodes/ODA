...
Gender <- c(rep("F",length(SBP)/2),rep("M",length(SBP)/2))
df <- data.frame(SBP,Gender,disease)
require(ggplot2)
fig2<-ggplot(df, aes(x = factor(disease), y=SBP, fill=Gender)) +  geom_dotplot(binaxis = "y", stackdir = "center",binpositions="all",binwidth = 3) + theme_bw() + ggtitle("Figure 2. Simulated Distribution of CV Disease in Patients (n=50)") + geom_hline(yintercept=145,lty=2) + ylab("Systolic Blood Pressure (mmHg)") + xlab("CV Disease (0=no,1=yes)")
print(fig2)
